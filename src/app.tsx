...
export function createApp({ database, captcha, jwtSecret = process.env.JWT_SECRET ?? "development-secret" }: {
  ...
  const app = new Hono();
  app.get("/", (c) => c.html(<Layout><main class="container mx-auto p-8"><h1 class="text-4xl font-bold">Website</h1><nav class="mt-6 flex gap-3"><a class="btn btn-primary" href="/auth">Auth</a><a class="btn btn-secondary" href="/events">Events</a><a class="btn btn-accent" href="/mailer">Mailer</a></nav></main></Layout>));
