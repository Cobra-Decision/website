...
export function SchemaTable({ database, query = {} }: { database: Database; query?: Record<string, string> }) {
  ...
  return <div id="schema-table">
    <form class="mb-4 flex flex-wrap items-end gap-3" hx-get="/dashboard/admin/report" hx-target="#schema-table" hx-swap="outerHTML">
      <label class="form-control"><span class="label-text">Search field</span><select class="select select-bordered select-sm" name="schema_field">{fields.map((name) => <option value={name} selected={field === name}>{name.replaceAll("_", " ")}</option>)}</select></label>
      <label class="form-control min-w-52 flex-1"><span class="label-text">Search value</span><input class="input input-bordered input-sm" name="schema_q" value={query.schema_q ?? ""} placeholder="Search schema" /></label>
      <button class="btn btn-primary btn-sm">Search</button><a class="btn btn-ghost btn-sm" href="/dashboard/admin/report">Reset</a>
    </form>
    <div class="overflow-x-auto rounded-box border border-base-300"><table class="table table-sm"><thead><tr>{fields.map((name) => <th><button type="button" class="btn btn-ghost btn-xs -ml-2" hx-get={url(name)} hx-target="#schema-table" hx-swap="outerHTML">{name === "table_name" ? "Table" : name === "name" ? "Column" : name.replaceAll("_", " ")}{sort === name ? direction === "asc" ? " ↑" : " ↓" : ""}</button></th>)}</tr></thead><tbody>{filtered.length ? filtered.map((row) => <tr><td class="font-medium">{row.table_name}</td><td>{row.name}</td><td><code>{row.type}</code></td><td>{row.notnull ? "Yes" : "No"}</td><td>{row.dflt_value ?? "—"}</td><td>{row.pk ? "Primary" : ""}</td></tr>) : <tr><td colSpan={fields.length} class="py-8 text-center opacity-60">No matching schema columns.</td></tr>}</tbody></table></div>
  </div>;
}
